...
Surface::~Surface()
{
	if (ownBuffer) _aligned_free(pixels); // free only if we allocated the buffer ourselves
}
...
void Surface::Plot(int x, int y, unsigned int c)
{
	if (x < 0 || y < 0 || x >= width || y >= height) return;
	pixels[x + y * width] = c;
}

void Surface::Print(const char* s, int x1, int y1, unsigned int c)
{
	if (!fontInitialized)
	{
		InitCharset();
...
void Surface::Line(float x1, float y1, float x2, float y2, unsigned int c)
{
