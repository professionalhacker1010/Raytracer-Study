...
public:
	...
	Buffer() : hostBuffer(0) {}
	Buffer(GLFWwindow* window, unsigned int N, void* ptr = 0, unsigned int t = DEFAULT);
	~Buffer();
...
public:
	// constructor / destructor
	Kernel(char* file, char* entryPoint, GLFWwindow* window);
	...
	void Run(Buffer* buffer, const int2 localSize = int2(32, 2), cl_event* eventToWaitFor = 0, cl_event* eventToSet = 0, cl_event* acq = 0, cl_event* rel = 0); //used to use make_int2
...
private:
	...
	void SetArgument(int idx, Vec2); //todo check if Vec2 instead of float2 ok
	void SetArgument(int idx, Vec3); //same
	void SetArgument(int idx, Vec4); //same
	// other methods
public:
	static bool InitCL(GLFWwindow* window);
