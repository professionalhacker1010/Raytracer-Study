...
// default worksize
static size_t workSize[] = { WIDTH, HEIGHT };
...
// Buffer constructor
// ----------------------------------------------------------------------------
Buffer::Buffer(GLFWwindow* window, unsigned int N, void* ptr, unsigned int t)
{
	if (!Kernel::clStarted) Kernel::InitCL(window);
	type = t;
	ownData = false;
...
// Kernel constructor
// ----------------------------------------------------------------------------
Kernel::Kernel(char* file, char* entryPoint, GLFWwindow* window)
{
	if (!clStarted) InitCL(window);
	// load a cl file
...
// InitCL method
// ----------------------------------------------------------------------------
bool Kernel::InitCL(GLFWwindow* window)
{
...
void Kernel::SetArgument(int idx, Vec2 value) { CheckCLStarted(); clSetKernelArg(kernel, idx, sizeof(Vec2), &value); }
void Kernel::SetArgument(int idx, Vec3 value) { CheckCLStarted(); Vec4 tmp(0, value[2], value[1], value[0]); clSetKernelArg(kernel, idx, sizeof(Vec4), &tmp); }
void Kernel::SetArgument(int idx, Vec4 value) { CheckCLStarted(); clSetKernelArg(kernel, idx, sizeof(Vec4), &value); }
